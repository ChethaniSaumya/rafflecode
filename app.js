...
async function main() {
    ...
    app.get('/profile', async function (req, res) {
        if (req.isAuthenticated()) {
            ...
            for (let i = 0; i < tickets.length; i++) {
                ...
                events.push(event);
                if (tickets[i].bought) {
                    boughtTickets = boughtTickets + 1;
                    boughtEvents.push(event._id);
                }
            }
            console.log(boughtEvents);
            const numberBoughtEvents = Array.from(new Set(boughtEvents));
            console.log(numberBoughtEvents);
            res.render('Profile', { user: req.user, tickets: tickets, events: events, boughtTickets: boughtTickets, boughtEvents: numberBoughtEvents.length, auth: req.isAuthenticated() });
        } else {
            res.redirect("/login")
        }
    });
    ...
    app.get('/addToCart/:eventid/:ticketvalue', async function (req, res) {
        if (req.isAuthenticated()) {
            ...
            const token = await Ticket.findOne({ $and: [{ eventId: req.params.tokenid }, { bought: true }] })
            if (token) {

                res.render('cart', { tickets: null, user: req.user, msg: "Tickets Already Bought :(" });
                return
            }

            try {
                const tokenFound = await Ticket.findOne({ $and: [{ value: req.params.ticketvalue }, { userId: req.user._id }, { eventId: req.params.eventid }] })
                console.log({ tokenFound }, { bought: false })
                if (tokenFound) {
                    ...
                    res.render('cart', { tickets: tickets, user: req.user, msg: "Ticket Is Already In Cart :(" });
                } else {
                    const ticket = new Ticket({
                        _id: Number(ids.ticketId),
                        userId: Number(req.user._id),
    ...
    app.get('/cart', async function (req, res) {
        if (req.isAuthenticated()) {
            const tickets = await Ticket.find({ userId: Number(req.user._id), bought: false });
            res.render('cart', { tickets: tickets, user: req.user, msg: "" });
        } else {
            res.redirect('/login');
        }
    });
    ...
    app.get('/endEvent/:eventId', async function (req, res) {
        if (req.isAuthenticated()) {
            ...
            const existance = event.userIds.length === 1 && event.userIds[0] === 0;
            if (!existance) {
                ...
                await Event.updateOne({ _id: Number(req.params.eventId) }, { eventEnd: true, winnerId: 0 });
            }
    ...
    app.post('/buyTickets', async function (req, res) {
        ...
        if (Object.keys(data).length === 0) {
            const tickets = await Ticket.find({ userId: Number(req.user._id), bought: false });
          return  res.render('cart', { tickets: tickets, user: req.user, msg: `No Data Found !` });
        } 

        const pairedData = [];
        const keys = Object.keys(data);

        if (Array.isArray(data.eventId)) {
        ...
        for (const item of pairedData) {
            const token = await Ticket.findOne({ $and: [{ eventId: item.eventId }, { value: item.value }, { bought: true }] });
            console.log({ token });
            ...
            if (invalidTokens.length > 0) {
                const tickets = await Ticket.find({ userId: Number(req.user._id), bought: false });
                res.render('cart', { tickets: tickets, user: req.user, msg: `Ticket(s) Already Bought for value(s): ${invalidTokens.map(token => token.value).join(', ')}` });
            } else {
                res.render('payment', { user: req.user, key: process.env.STRIPEPUBLISHABLEKEY, key2: process.env.STRIPEACCOUNTID });
            }
